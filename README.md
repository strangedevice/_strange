...
1. Search for `'_strange'` (inside single quotations) to capture the text domain.
2. Search for `_strange_` to capture all the function names.
3. Search for ` _strange` (with a space before it) to capture DocBlocks.
4. Search for `_s-` to capture prefixed handles.

OR

* Search for: `'_strange'` and replace with: `'megatherium'`
* Search for: `_strange_` and replace with: `megatherium_`
